...
import com.tpi135_2023.Ingenieria.occ.ues.ued.sv.deliverytpi.control.DireccionBean;
...
@Path("/direccion")
public class DireccionResource extends AbstracRestResources<Direccion>{

    @Inject
    DireccionBean db;

    @Override
    protected AbstractDataAccess<Direccion> getDataAccesBean() {
        return this.db;
    }
    ...
    public Response ingresar(Direccion entidad){
        ...
        try {
            Direccion entidadCreada = this.getDataAccesBean().Ingresar(entidad);
            rb = Response.status(Response.Status.CREATED);
            rb.header("Content-Type", MediaType.APPLICATION_JSON);
            rb.header("location", "direccion/" + entidadCreada.getIdDireccion());
    ...
    public Response buscarPorId(@PathParam("id") int id) {
        Direccion cEntity = null;

        Response.ResponseBuilder rb;
        try {
            cEntity = db.traerIdPorDireccion(id);
            rb = Response.ok();
            rb.header("Content-Type", MediaType.APPLICATION_JSON);
            rb.entity(cEntity);
            return rb.build();
