 ...
 */
package com.tpi135_2023.Ingenieria.occ.ues.ued.sv.deliverytpi.entity;
...
public class ComercioIT {
    ...
    static final MountableFile warDelivery = MountableFile.forHostPath(Paths.get("target/deliverytpi.war").toAbsolutePath(), 0777);
     ...
     */
    @ClassRule
    public static PostgreSQLContainer<?> postgreSql = new PostgreSQLContainer<>(DockerImageName.parse("postgres:13-alpine"))
            ...
            .withCreateContainerCmdModifier(cmd -> cmd.withName("Postgres-Container"));
//            .waitingFor(Wait.forLogMessage(".* PostgreSQL init process complete .*\\s", 1));
    @ClassRule
    public static GenericContainer<?> payara = new GenericContainer<>(DockerImageName.parse("payara/micro:6.2023.2-jdk17"))
            .withCopyFileToContainer(warDelivery, "/opt/payara/deployments/deliverytpi.war")
            .withExposedPorts(8080)
            .waitingFor(
                    Wait.forLogMessage(".* Payara Micro .* ready in .*\\s", 1)
            )
            .withEnv("DB_USER", USER_DB)
            .withEnv("DB_PASSWORD", PASSWORD_DB)
            .withEnv("POSTGRES_DB", NAME_DB)
            .withStartupTimeout(Duration.ofSeconds(300))
            .withCommand("--deploy /opt/payara/deployments/deliverytpi.war --contextRoot /")
            .withNetwork(network)
    ...
    public static void lanzarPayaraTest() {
        System.out.println("Comercio - lanzarPostgrest");
        postgreSql.start();
        System.out.println("Comercio - lanzarPayara");
        payara.withEnv("DB_JDBC_URL",
                String.format("jdbc:postgresql://%s:%d/%s",
                        "db",
                        postgreSql.getMappedPort(5432),
                        postgreSql.getDatabaseName())
        );
        System.out.println(postgreSql.getMappedPort(5432));
        
        payara.start();
        System.out.println(postgreSql.getLogs());
        System.out.println(payara.getLogs());
        
        //Inicializando las entradas 
        endpoint = "http://" + payara.getHost() + ":" + payara.getMappedPort(8080) + "/deliverytpi/";
    ...
    public void crearTest() {
        Assertions.assertTrue(postgreSql.isRunning());
